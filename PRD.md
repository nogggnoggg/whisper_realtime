...
- 支援中文與英文雙向語音對話。
- 架構需支援未來擴充語言對：以兩個語言選單（語言 A ↔ 語言 B，各可選 韓文／中文／英文）組合決定互譯方向（詳見 §7.10）。
- 即時顯示原文與翻譯。
- 支援「精準翻譯」第三行顯示。
- 保留對話上下文，避免前文太快消失。
...
- 不做進階降噪或自動環境噪音校準。
- 不實作韓文與語言對選單（Phase 3），但程式與資料模型不得寫死「僅支援中英」（見 §7.10、§9.4）。
...
## 7.10 多語言對（Phase 3 規劃）

### 設計

不採用固定模式列舉，而是**兩個語言選單**自由組合：
...
- 每個選單可選：韓文（한국어）／中文／英文。
- 組合即決定互譯方向，例如 韓↔中、韓↔英、中↔英（預設，= 現行為）。
- Top bar 的「中文 ↔ English」改為反映目前選擇的語言對。

### 語言偵測通用化原則

§7.9 的偵測邏輯通用化為：「判定本次發言屬於語言 A 還是語言 B，翻譯成另一邊」。各語言以字元區段判定：

| 語言 | 字元區段 |
|---|---|
| 中文 | U+4E00–U+9FFF、U+3400–U+4DBF（漢字） |
| 韓文 | U+AC00–U+D7AF（諺文音節）、U+1100–U+11FF（諺文字母） |
| 英文 | 拉丁字母為多數且不含上述區段 |

### 屆時需要修改的 zh/en 寫死點（實作備忘）

| 位置 | 現況 |
|---|---|
| `server/lang.js` | CJK 二元偵測（韓文會被誤判為 en） |
...
- Phase 1–2：不實作 UI 與韓文，但 Phase 2 的資料模型必須以語言對為一級概念（§9.4）。
- Phase 3：實作兩選單 UI、韓文偵測與翻譯、韓文 STT／翻譯品質驗證（gpt-realtime-whisper 韓文轉錄品質需獨立測試）。
...
> ⚠️ Schema 設計原則（Phase 2 必守）：glossary 與 translation logs 必須以 `(source_lang, target_lang)` 語言對為一級欄位，語言值用標準代碼（zh / en / ko …），不得隱含「只有中英」的假設 — 為 Phase 3 韓文擴充預留（見 §7.10）。
...
- 韓文支援 + 語言對雙選單（韓/中/英 任兩種互譯，見 §7.10）。
- Glossary 管理頁。
- 基本登入。
- 多站別 / 產線設定。
- Safety keyword 標示。
